...
RegisterCommand("group", function (source, args)
	...
	if not args[1] or not args[2] or not args[3] then
		return TriggerClientEvent("Notify",source,"vermelho","Parâmetros inválidos! <br>Use group {id} {grupo} {nível}","Erro!",5000)
	end

	if Passport < 10 or vRP.HasGroup(Passport, "Admin") then

		local FindGroup 			= vRP.Groups()[args[2]]
		local FindOtherPassport     = vRP.Identity(args[1])
		local Hierarchy 			= FindGroup.Hierarchy[tonumber(args[3] or 1)]

		if FindGroup and FindOtherPassport and Hierarchy then
			
			vRP.SetPermission(FindOtherPassport, args[2], Hierarchy)

			return TriggerClientEvent("Notify",source,"verde","Grupo <b>"..args[2].."</b> Setado no nível de <b>"..Hierarchy.."</b>","Sucesso",5000)
		
		end

		return TriggerClientEvent("Notify",source,"vermelho","Grupo não existe","Erro!",5000)
