...
AddStateBagChangeHandler(nil, ('player:%s'):format(LocalPlayer.state.handle), function(bagName, key, value, reserved, replicated)
    print(("LocalPlayer state '%s' mudou para: %s"):format(key, tostring(value)))
end)

AddStateBagChangeHandler('Admin', nil, function(bagName, key, value)
    local netId = tonumber(bagName:match("player:(%d+)"))
    if netId then
        print(("Player %d teve Admin alterado para: %s"):format(netId, tostring(value)))
    end
end)